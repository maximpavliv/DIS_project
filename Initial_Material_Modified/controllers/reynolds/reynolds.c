...
//From this lab (doesn't seem to work very well)
...
//From lab 2 (braitenberg) (worked well there)
        ...
        for (j=0;j<2;j++) 
	{
        ...
        if(MIGRATORY_URGE == 0){
          ...
          speed[robot_id][1] += 0.01*sin(loc[robot_id][2] + M_PI/2);
            ...
            speed[robot_id][0] += MIGRATION_WEIGHT*(migr[0]-
              loc[robot_id][0]);
            speed[robot_id][1] -= MIGRATION_WEIGHT*(migr[1]-
...
void initial_pos(void){
	...
	while (initialized[robot_id] == 0) {
		...
		// Only info about self will be taken into account at first.
                      ...
                      //REINITIALISING THE MIGRATORY URGE MANUALLY
                      migr[0] = 10.0;
                      migr[1] = 0.0;
